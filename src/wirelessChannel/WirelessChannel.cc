...
void WirelessChannel::initialize(int stage)
{
	...
	this->seed = par("seed").longValue();
	
	this->k = par("K");
	this->k_sigma = par("K_sigma");
	trace() << "K: " << this->k << " k_sigma: " << this->k_sigma;
	int seed_normal;
	...
	if(this->seed == 0){
		...
		seed_normal = this->seed;
	}	
	
	std::mt19937 gen2(seed_normal);
	...
	varrice = new double[16*(numOfSpaceCells+1)*(numOfSpaceCells+1)];
	
	meanrice =  new double[16*(numOfSpaceCells+1)*(numOfSpaceCells+1)];
	
	for(int i = 0; i < 16*(numOfSpaceCells+1)*(numOfSpaceCells+1); i++) {
		varrice[i] = 1;
		meanrice[i] = 0;
	}
	...
	//trace() << "Ruan - numOfSpaceCells: " << numOfSpaceCells << " distanceThreshold: " << distanceThreshold;

	for (int i = 0; i < numOfSpaceCells; i++) {
		trace() << "Analise para o no " << i;
		if (onlyStaticNodes) {
			x1 = nodeLocation[i].x;
			y1 = nodeLocation[i].y;
			z1 = nodeLocation[i].z;
	...
	//	trace() << "Ruan - pathLoss[i].push_front(new PathLossElement(i, 0.0)) - " << i;
		totalElements++;	//keep track of pathLoss size for reporting purposes
	//	trace() << "Ruan - totalElements++ " << totalElements;
		...
		//	trace() << "Dist para o no " << j << ": " << dist;
			...
			if (dist < d0/10.0) {
				...
				//trace() << "Ruan - dist: " << dist << " totalElements: " << totalElements;
	...
	//	trace() << "Ruan - modelo temporal ativado";
	} else {
...
void WirelessChannel::handleMessage(cMessage * msg)
{
	...
	case WC_SIGNAL_START:{
		...
		/*	currentTime = simTime();
			...
			generatePowerFading(srcAddr,(int)(signalMsg->getCarrierFreq()-2405)/5); //update the value of received power for each conecction using the Rice distribution
			...
			for (it1 = pathLoss[cellTx].begin(); it1 != pathLoss[cellTx].end(); it1++) {
				 ...
				 */
				float currentSignalReceived = signalMsg->getPower_dBm() - (*it1)->avgPathLoss[(int)((signalMsg->getCarrierFreq()-2405)/5)];
			//	trace() << "Frequencia em uso: " << signalMsg->getCarrierFreq();
			//	trace() << "Canal em uso: " << (int)((signalMsg->getCarrierFreq()-2405)/5) << "Pathloss: " << currentSignalReceived;
...
//This function will update a pathloss element for given source and destination cells with a given value of pathloss
//If this pair is already defined in pathloss array, the old value is replaced, otherwise a new pathloss element is created
void WirelessChannel::updatePathLossElement2(int src, int dst, float shadowing, int channel)
{
	float dist, x1,x2,y1,y2,z1,z2, PLd, bidirectionalPathLossJitter;
	
	x1 = nodeLocation[src].x;
	y1 = nodeLocation[src].y;
	z1 = nodeLocation[src].z;
	
	x2 = nodeLocation[dst].x;
	y2 = nodeLocation[dst].y;
	z2 = nodeLocation[dst].z;
	...
	//trace() << "Distância de limiar - update: " << distanceThreshold;
	...
	if (dist < d0/10.0) {
		...
		PLd = PLd0 + 10.0 * pathLossExponent * log10(dist / d0) - shadowing;//+ normal(0, sigma);
		//trace() << "Ruan - Atualiza pL - dist : " << dist << " de " << src << " a " << dst << ": " << PLd;
		bidirectionalPathLossJitter = normal(0, bidirectionalSigma) / 2;
	}

	//if (maxTxPower - PLd - bidirectionalPathLossJitter >= signalDeliveryThreshold) {
	//	pathLoss[src].push_front(new PathLossElement(dst,PLd + bidirectionalPathLossJitter));
		// << "Atualizou uma direcao";
		list <PathLossElement*>::iterator it1;
		for (it1 = pathLoss[src].begin(); it1 != pathLoss[src].end(); it1++) {
			if ((*it1)->cellID == dst) {
				(*it1)->avgPathLoss[channel] = PLd + bidirectionalPathLossJitter;
				break;
			}
		}
		//}
...
//	if (maxTxPower - PLd + bidirectionalPathLossJitter >= signalDeliveryThreshold) {
	/*	trace() << "Atualizou outra direcao";
		//list <PathLossElement*>::iterator it1;
		for (it1 = pathLoss[dst].begin(); it1 != pathLoss[dst].end(); it1++) {
			if ((*it1)->cellID == src) {
				(*it1)->avgPathLoss[channel] = PLd - bidirectionalPathLossJitter;
				break;
			}
...
//p is the probability of changing the channel's characteristics
bool WirelessChannel::updateChannel(double p) {

	bool changed = false;
	int x;
	double shad=0;
	double shadowing=0;
	static bool pri = false;
	
    std::normal_distribution<> gaussian(0,sigma);
    std::normal_distribution<> gaussiank(0,k_sigma);
...
//	trace() << "Valor sorteado: x=" << x;
	if(!pri || x == 0 || simTime() == 0) {
		pri = true;
		//muda características de todos
		//updating the channels' characteristcs
		for(int c = 0; c < 16; c++) { //para cada canal 
			for (int i = 0; i < numOfSpaceCells; i++) {
				//trace() << "Atualiza caracteristicas - int i:" << i;
				for (int j = 0; j < numOfSpaceCells; j++) {
				    while(true) {
				  	  shad = std::round(gaussian(gen));
					  if(shad >= -40 && shad <= 40) break;
				    }
	
					//trace() << "New shad between " << i << " and " << j << ": " << shad;
					double k_factor = k + std::round(gaussiank(gen));
				//	trace() << "New K" << i << " and " << j << ": " << k_factor;
				
					k_factor = pow(10,(k_factor/10));
					
					varrice[16*(j+i*numOfSpaceCells)+c] = k_factor;//rand()%20 + 1;
					//varrice[16*(j+i*numOfSpaceCells)+c] /= 10.0;
					meanrice[16*(j+i*numOfSpaceCells)+c] = shad;
				  //  trace() << "Shadow de " << i << " para " << j << " no canal " << c << ": " << shad;
				//	ric_rng.setup(varrice[c][i][j],meanrice[c][i][j]);
					changed = true;
				}
			}
		}
		trace() << "Mudança de parametros..." << shad;
	}	
	return changed;
}

void WirelessChannel::generatePowerFading(int i, int c) {	
    Rice_RNG ric_rng(1,0);
	
	for (int j = 0; j < numOfSpaceCells; j++) {
		Rice_Fading_Generator rfg(1);
		rfg.init();
		//rfg.set_LOS_power(k_factor);
		//ric_rng.setup(varrice[16*(j+i*numOfSpaceCells)+c],meanrice[16*(j+i*numOfSpaceCells)+c]);
		double valrice;//= ric_rng.operator()();
		rfg.set_LOS_power(varrice[16*(j+i*numOfSpaceCells)+c]);
		valrice =  meanrice[16*(j+i*numOfSpaceCells)+c] + 10*log10(pow(abs(rfg.generate(1)(0)),2));//10*log10(valrice);
	//	trace() << "Generating with average " << meanrice[16*(j+i*numOfSpaceCells)+c];
		updatePathLossElement2(i,j,valrice,c);
	}
}
