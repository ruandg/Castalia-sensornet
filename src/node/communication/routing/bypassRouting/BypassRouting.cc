...
void BypassRouting::fromApplicationLayer(cPacket * pkt, const char *destination)
{
	...
	trace() << "getSource -> " << netPacket->getSource();
	netPacket->setDestination(destination);
	trace() << "getDestination -> " << netPacket->getDestination();
...
void BypassRouting::fromMacLayer(cPacket * pkt, int srcMacAddress, double rssi, double lqi)
{
	...
	if (netPacket) {
		...
		trace() << "SELF destination: " << destination.compare(SELF_NETWORK_ADDRESS);
		trace() << "Broadcast: " << destination.compare(BROADCAST_NETWORK_ADDRESS);
...
/*void BypassRouting::handleNetworkControlCommand(cMessage* netCmd)
{
	int cont_ch = netCmd->getKind();
	double new_channel = 2405.0 + (cont_ch%16)*5;
	trace() << "Novo canal: " << new_channel;
	trace() << "contorle recebido em rede...";
}
*/