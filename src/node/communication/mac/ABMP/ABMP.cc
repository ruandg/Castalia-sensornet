...
void inline ABMP::medianValue(uint8_t node_addr) {
    int temp;
...
void inline ABMP::mediana(uint8_t node_addr) {
...
void ABMP::calculatePRRu(uint8_t node_addr) { 
...
void ABMP::calculatePRRd(uint8_t node_addr) {
...
void ABMP::fromNetworkLayer(cPacket * pkt, int destination)
{
	//if(SELF_MAC_ADDRESS!=8 && SELF_MAC_ADDRESS!=0) return;
...
void ABMP::fromNetworkLayer(cPacket * pkt, int destination, bool ack)
{
	//if(SELF_MAC_ADDRESS!=8 && SELF_MAC_ADDRESS!=0) return;
 ...
 */
void ABMP::fromRadioLayer(cPacket * pkt, double rssi, double lqi)
{
//	//if(SELF_MAC_ADDRESS!=8 && SELF_MAC_ADDRESS!=0) return;
	
	ABMPPacket *macPkt = dynamic_cast <ABMPPacket*>(pkt);
	if (macPkt == NULL)
		return;

	if(macPkt->getKindDSME() == BEACON_PACKET && waiting_beacon) {
		...
		setTimer(BEGIN_CFP, ((slotSize-0.212)/0.1));
...
void ABMP::timerFiredCallback(int index)
{
	////if(SELF_MAC_ADDRESS!=8 && SELF_MAC_ADDRESS!=0) return;
	...
	switch (index){
		...
		case SEND_BEACON: {	
			...
			ABMPPacket *macBeacon; 
			macBeacon = new ABMPPacket("ABMP beacon", MAC_LAYER_PACKET);
			...
			setTimer(SEND_BEACON, ((QTDNODES*slotSize)/0.1));	//32 -> two superframes
			
			setTimer(BEGIN_CFP, (slotSize/0.1)); //goes to time slot 1
		...
		case BEGIN_CFP: {
			...
			//buffer of new packets
				...
				ABMPPacket *macFrame = new ABMPPacket("DSME packet", MAC_LAYER_PACKET);
			...
			if(cfp_cont < (QTDNODES-1)) { //31 -> two superframes (extract the 8 from the CAP)
		...
		case END_FRAME: {	
			...
			/*	int ch_beacon = beacon_id%blacklist_size;
				...
				setTimer(BEGIN_CFP, ((slotSize+1)/0.1));
				waiting_beacon = false;
				beaconReceived = true;
				cfp_cont = 0;
			}
